...
def get_vehicles_with_locations():
    """Get all vehicles with their latest locations, automatically refresh from Tesla API"""
    try:
        # Check if skip_refresh parameter is provided to skip Tesla API call
        skip_refresh = request.args.get('skip_refresh', 'false').lower() == 'true'
        
        vehicles = get_all_vehicles()
        vehicles_with_locations = []
        
        # Get Tesla instance for fresh location data (unless skipped)
        tesla = None
        ...
        if not skip_refresh:
            ...
            if tesla:
                ...
                print("🔄 Auto-refreshing locations from Tesla API...")
            else:
        ...
        for vehicle in vehicles:
            ...
            # Automatically refresh location from Tesla API if connection available
            if refresh_from_api and tesla and vehicle_id:
                try:
                    print(f"🔄 Fetching fresh location for vehicle {vehicle_id}")
        ...
        return jsonify({
            ...
            "success": True,
            "refreshed_from_api": refresh_from_api,
            "message": "Auto-refreshed from Tesla API" if refresh_from_api else "Using database data only"
        })
    except Exception as e:
